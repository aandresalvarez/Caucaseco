...
$criaderos= array();
exec("..\python\Buscar_error.py",$criaderos);
...
$lat_criaderos[$j]= (float)(str_replace(",",".",$criaderos[$i+1])) ;                      //(string)$criaderos[$i+2];
$long_criaderos[$j]=abs((float)(str_replace(",",".",$criaderos[$i+2])))*-1; 
$sitio[$j]=trim((string)$criaderos[$i+3]);
...
function initialize() {
  ...
  var icono_criadero='../markers/mosquito_breeding/bullet_blue.png';
   <?php	  
	...
	for($j=0;$j<count($long_criaderos);$j++)
	{
	 echo"var contenido='<div  style=\'width: 160px; \'> Codigo:</br> <a href=\'http://claimproject.com/redcap570/redcap_v6.9.4/DataEntry/index.php?pid=192&page=inmaduros&id=$id_criaderos[$j]\'> $id_criaderos[$j] </a> <br>Tipo:</div>';";
