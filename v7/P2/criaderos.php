...
$criaderos= array();
exec("..\python\CriaderosMarzo.py",$criaderos);
...
$lat_criaderos[$j]= (float)(str_replace(",",".",$criaderos[$i])) ;                      //(string)$criaderos[$i+2];
$long_criaderos[$j]=abs((float)(str_replace(",",".",$criaderos[$i+1])))*-1; 
$id_criaderos[$j]=trim((string)$criaderos[$i+2]);
...
<html>
  <head>
    <meta charset="utf-8">
    <title>CLAIM- P2 Breeding Sites Transversal</title>
    <style>
   ...
   <?php	  
	...
	for($j=0;$j<count($long_criaderos);$j++)
	{
	   ...
	   //marca puntos en el mapa
		...
		echo"var contenido='<div  style=\'width: 160px; \'> Codigo:</br> <a href=\'http://claimproject.com/redcap570/redcap_v6.9.4/DataEntry/index.php?pid=86&page=p2_criaderos&id=$id_criaderos[$j]\'> $id_criaderos[$j] </a> ';";	
